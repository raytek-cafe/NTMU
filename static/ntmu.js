...
var page = {
    ...
    navigate: function(urlParts) {
        ...
        var fetchData = function(url, successCallback, errorCallback) {
            if (isIE) {
        ...
        switch (urlParts[0]) {
            case "":
                template = "home";
                fetchData("data/packs.yml?t=" + Date.now(), function(packsText) {
                    try {
                        var packs = jsyaml.load(packsText);
                    ...
                    template = "error";
                    data.message = "Data request for packs list failed.";
                    self.renderTemplate(template, data);
                });
                break;
            ...
            case "pack":
                ...
                fetchData("data/" + id + "/pack.yml?t=" + Date.now(), function(packDataText) {
                    try {
                        var packData = jsyaml.load(packDataText);
                        data.pack = packData;
                        data.pack.id = id;
